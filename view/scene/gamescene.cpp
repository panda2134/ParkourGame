            ...
            if ((--hotbarTicksLeft) >= 0) {
                ...
                if (item != nullptr) {
                    ...
                    p.drawText(QRect(hotbarTargetStart - QPoint(0, fontSize + marginBottom),
                                   QSize(hotbarTargetRect.size().width(), fontSize)),
                        item->getDisplayName(), Qt::AlignCenter | Qt::AlignHCenter);
                }
            }
        }
    } else {
...
void GameScene::calculate() {
    if (mode == SceneMode::GAMING && ingameGUI == nullptr) {
...
void GameScene::CameraInfo::updateViewport() {
    ...
    if (movingTicksLeft <= 0) {
        ...
        if (!farMove) {
            xMinOfViewport = cameraStart + (cameraEnd - cameraStart) * ease.valueForProgress((CAMERA_MOVE_TICKS - movingTicksLeft) * 1.0 / CAMERA_MOVE_TICKS);
        } else {
            //按照二次函数移动相机, a为二次项系数
...
void GameScene::showGUI(QSharedPointer<GUIBase> gui, Qt::Key exitKey) {
    this->ingameGUI = gui;
    auto* guiEventFilter = new GUIEventFilter(exitKey, gui.data());
    connect(gui.data(), SIGNAL(quitGUI()), this, SLOT(quitGUI()));
    connect(guiEventFilter, SIGNAL(quitGUI()), this, SLOT(quitGUI()));
    gui->installEventFilter(guiEventFilter);
}
}
